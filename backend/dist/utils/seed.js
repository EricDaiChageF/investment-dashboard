...
const seedData = async () => {
    ...
    if (userCount === 0) {
        ...
        await new Promise((resolve, reject) => {
            db.run('INSERT INTO users (username, password_hash) VALUES (?, ?)', ['admin', passwordHash], function (err) {
                if (err)
                    reject(err);
                else {
    ...
    if (stockCount === 0) {
        ...
        for (const stock of defaultStocks) {
            await new Promise((resolve, reject) => {
                db.run('INSERT INTO stocks (code, name, category, shares, avg_price, market) VALUES (?, ?, ?, ?, ?, ?)', [stock.code, stock.name, stock.category, stock.shares, stock.avg_price, stock.market], function (err) {
                    if (err)
                        reject(err);
                    else
                        resolve(null);
